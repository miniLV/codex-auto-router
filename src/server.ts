...
function html(view: UsageViewModel, nonce: string, loading = false, debugLines?: string[]): string {
  ...
  const dynamicStyles = `${MODEL_COLORS.map((color, index) => `.model-swatch[data-color="${index}"]{background:${color}}`).join("")}${officialUsage ? `.usage-progress span{transform:scaleX(${usedPercent / 100})}.reset-progress span{transform:scaleX(${1 - (reset?.remainingPercent ?? 0) / 100})}` : ""}.readiness.is-empty{display:none}.setup-status.is-ready{display:flex;gap:8px;margin-top:14px;padding:0 2px;border:0;background:transparent;box-shadow:none;color:var(--muted)}.setup-status.has-issues{border-color:var(--orange);background:#fff1a8}.setup-item{margin:5px 0 0}.setup-link{color:var(--blue);font-weight:800;text-decoration:underline}.debug-log-panel{margin-top:18px;border-color:var(--ink);background:var(--panel)}.debug-log-header{display:flex;align-items:center;justify-content:space-between;gap:12px}.debug-log{max-height:220px;overflow:auto;margin:10px 0 0;padding:10px;border:1px solid rgba(31,35,40,.2);border-radius:8px;background:rgba(31,35,40,.04);font:12px/1.5 ui-monospace,SFMono-Regular,Menlo,monospace;white-space:pre-wrap}.debug-log:focus{outline:2px solid var(--blue);outline-offset:2px}.debug-log-panel .debug{margin-top:10px}.debug-log-panel .debug-copy{padding:6px 9px;font-size:11px}`;
